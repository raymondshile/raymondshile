# Hi, I'm Olashile 👋

Cloud Infrastructure student focused on Linux, systems administration, and enterprise technologies.

Selected for a Technical Support Engineer Internship at Red Hat.

I build and troubleshoot real-world systems with a focus on reliability, performance, and automation.

---

## What I’m Focused On

- Linux system troubleshooting and performance tuning  
- System log analysis (journalctl, systemctl, top, netstat)  
- Virtualization and infrastructure (Proxmox, VMware)  
- Active Directory and enterprise environments  
- Automation using Bash and Python  
- Preparing for RHCSA certification  

---

##  Projects

###  Linux Server Monitoring Tool 
[![Monitoring Tool](https://raw.githubusercontent.com/raymondshile/linux-server-monitoring-bash/main/monitoring-tool.png)](https://github.com/raymondshile/linux-server-monitoring-bash)

Built a Bash-based monitoring tool to track system performance and support troubleshooting.

- Monitors CPU, memory, disk usage, and system health  
- Automates log analysis for faster issue detection  
- Improves system visibility and performance tracking  

 https://github.com/raymondshile/linux-server-monitoring-bash


Detected repeated high CPU usage alerts during testing and investigated system processes using `top`. Adjusted monitoring thresholds in the config file to reduce false positives and improve alert accuracy.

---

###  Proxmox Virtual Lab
Designed and deployed a Proxmox-based lab to simulate enterprise infrastructure.

- Configured virtual machines, storage, and network bridges  
- Simulated real-world system and network issues  
- Troubleshot VM connectivity using networking tools  

---

###  Windows Server & Active Directory
Configured and managed Active Directory in a Windows Server environment.

- Created and managed users, groups, and organizational units  
- Applied Group Policy for system configuration and security  
- Simulated enterprise user management scenarios  

---

###  Virtualization & Networking
Built and managed virtual environments using VMware and VirtualBox.

- Deployed multiple virtual machines for testing  
- Configured basic networking between systems  
- Troubleshot connectivity and system behavior  

---

##  Technical Skills

### Systems
- Linux (RHEL, Ubuntu)  
- Windows Server  

### Infrastructure & Virtualization
- Proxmox  
- VMware  
- VirtualBox  
- Networking  

### Scripting & Tools
- Bash  
- Python  
- Git / GitHub  

### Core Skills
- Active Directory  
- System Troubleshooting  
- Log Analysis  

---

##  Career Goals

My goal is to grow into a Cloud Infrastructure Engineer or Site Reliability Engineer, working with large-scale systems, cloud platforms, and open-source technologies.

---

## 📅 Availability

Available for full-time opportunities in Linux system administration, cloud infrastructure, site reliability engineering, and technical support starting August 2026.
##  Support Engineering Mindset

- Focus on root cause analysis, not just quick fixes  
- Comfortable working with logs, system metrics, and CLI tools to diagnose issues  
- Approach problems methodically: reproduce, isolate, resolve, and document  
- Strong interest in Linux support environments and enterprise systems  
